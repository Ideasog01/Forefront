...
namespace XRLab
{
    ...
    public class XRIObjectComponentManager : MonoBehaviour
    {
        ...
        [SerializeField] private Text selectedObjectName;
        ...
        [SerializeField] private ObjectMaterialManager materialManager;
        [Tooltip("A Reference to the rigid body manager script, if left unassigned will attempt to automatially locate the script")]
        [SerializeField] private ObjectRigidBodyManager rigidbodyManager;
        ...
        void Awake()
        {
            if (materialManager == null) //if left unassigned will try and locate a material manager attached to the same game object
                materialManager = gameObject.GetComponent<ObjectMaterialManager>();

            if (rigidbodyManager == null)
                rigidbodyManager = gameObject.GetComponent<ObjectRigidBodyManager>();
        }
        ...
        private void UpdateObjectUI()
        {
            if (selectedObjectName != null)
                selectedObjectName.text = m_selectedGameObject.name;

            materialManager.SetSelectedObject(m_selectedGameObject, true);
            rigidbodyManager.SetSelectedObject(m_selectedGameObject, true);
        ...
        public void ListComponents(GameObject obj)
        {
            ...
            {
                output.text += i + ". " + m_selectedObjectComponents[i].GetType().Name + "\n";
            }
